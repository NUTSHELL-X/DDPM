...
def config_parser():
    ...
    parser.add_argument(
        "--config", is_config_file=True, help="config file path", default="config.yaml"
    )
    ...
    parser.add_argument(
        "--training_params_path", type=str
    )  # path to save training parameters(optimizor,total training epochs)
    parser.add_argument(
        "--save_images", type=bool, default=True
    )  # whether save images while training or not
    parser.add_argument(
        "--generated_image_folder", type=str
    )  # path to save generated images(.jpg file)
    parser.add_argument(
        "--generated_image_folder_test", type=str
    )  # path to save generated images when testing(.jpg file)
    parser.add_argument("--dataset_path", type=str)  # path containing dataset
    parser.add_argument(
        "--dataset_type", type=str, default="image_folder"
    )  # dataset type (see dataset.py)
    parser.add_argument("--num_steps", type=int, default=1000)  # T
    parser.add_argument("--epochs", type=int)  # training epochs
    parser.add_argument(
        "--dtype", type=str
    )  # data type used for training(float16,float32)
