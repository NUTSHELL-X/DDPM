...
if __name__ == "__main__":
    ...
    for i in range(img_num):
        ...
        with torch.no_grad():
            x_seq = p_sample_loop(
                model, (1, 3, h, w), num_steps, betas, one_minus_alphas_bar_sqrt
            )
        ...
        for step in range(0, num_steps // interval):
            ...
            save_image(
                cur_x,
                os.path.join(
                    args.generated_image_folder_test,
                    f"generated_img_{i}_{step*interval}.jpg",
                ),
            )
        ...
        save_image(
            x_seq[-1],
            os.path.join(
                args.generated_image_folder_test, f"generated_img_{i}_{num_steps-1}.jpg"
            ),
        )
